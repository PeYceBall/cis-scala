...
object task3 extends App {
  ...
  def combineErrors[E, A](a: Either[NonEmptyList[E], A])(implicit semigroupE: Semigroup[E]): Either[E, A] =
    a match {
      case Left(list) => Left(list.reduceLeft(semigroupE.combine))
  ...
  def mergeMaps[K, V](left: Map[K, V], right: Map[K, V])(implicit semigroupV: Semigroup[V]): Map[K, V] = {
    val combined = left.map { case (k, v) => if (right.contains(k)) (k, semigroupV.combine(v, right(k))) else (k, v) }
    combined ++ (right -- left.keySet)
  }
